...
df <- read.csv("https://raw.githubusercontent.com/RealityBending/FakeFace2/refs/heads/main/data/data_task.csv") |> 
  ...
  select(Participant, Sex=Gender, Condition, Item=Stimulus, Real, RealNoex, Attractive, Beauty)
...
```{r}
m <- lme4::lmer(Real ~ Sex / poly(Attractive, 2) * Condition + (1|Participant) + (1|Item), data = df)
display(parameters::parameters(m, effects = "fixed"))
...
make_ppcheck  <- function(m, fill = "#FF9800") {
  ...
  pred |>
    ...
    stat_bin(data = df, aes(x = Real, y = after_stat(density)),
...
```{r}
# c <- estimate_contrasts(m2_chocoat, contrast = "Condition", by = "Sex", predict = "mu")
# c <- estimate_contrasts(m2_chocoat, contrast = "Condition", by = "Sex", predict = "mu", backend = "emmeans")
# c2 <- estimate_contrasts(m2_chocoat, contrast = "Condition", by = "Sex", predict = "confright", backend = "emmeans")
```
...
p4_density <- pred$densities |> 
  mutate(Attractive = as.factor(Attractive),
         y = ifelse(x %in% c(0, 1), y * 3, y),  # Inflate extremes for visibility
         x = ifelse(Condition == "AI-Generated", x + 0.1, x)) |>  
